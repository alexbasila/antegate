#!/usr/bin/env python3
# =============================================================================
# ANTEGATE v0.1.0  -  antegate_v06.py
# -----------------------------------------------------------------------------
# Artifact provenance (belongs in the file, not in a chat log):
#   Script     : antegate_v06.py
#   Stage      : ANTEGATE_006 - evidence-based admission: admission decided on
#                verifiable, cryptographically bound evidence
#   Parents    : antegate_v05.py, antegate_v041.py
#   Created    : 2026-09-21
...
#   Run        : python antegate_v06.py --demo
#   Writes     : antegate/state_v06/ (append-only state, nothing overwritten)
#                antegate/ANTEGATE_006_REPORT_<UTC>.md
#
# Claim of this run, exactly this and no larger:
#   Admission to a shared AI trust domain can depend on independently verifiable
#   evidence rather than self-declaration alone. The resulting admission
#   credential can then govern access across independently operated services and
#   remain centrally revocable.
#
...
SCRIPT_NAME = "antegate_v06.py"
AUFTRAG_ID = "ANTEGATE_006"
AUTHORITY_ID = "ANTEGATE-CA-1"
PARENT_ARTIFACT = "antegate_v05.py (ANTEGATE_005) and antegate_v041.py (ANTEGATE_004 R1)"
...
def build_package(model_id: str, operator: Operator, issuer: EvidenceIssuer,
                  ...
                  mandate: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
def declaration_only_package(model_id: str) -> Dict[str, Any]:
    """The old world: self-declaration only, no evidence."""
    return {"model_id": model_id,
            "operator": {"name": "Operator-SelfDeclaration", "jurisdiction": "AT",
                         "contact": "claims@example.org"},
            "mandate": MANDATE, "requested_domain": DOMAIN_ID, "ttl_seconds": 3600,
            "law_code_acknowledged": True, "revocation_accepted": True,
            "evidence": {}}
...
def run_demo(root: Path, report_dir: Path) -> Path:
    ...
    say(f"Evidence-Issuer: {issuer.issuer_id}")
    ...
    def table(decision: Dict[str, Any]) -> None:
        for c in decision["criteria"]:
            say(f"      {c['criterion']}  {c['status']:11s} {c['evidence_type']:22s} "
                f"{c['evidence_id']}  {c['reason']}")

    # ---------------- PART 1: evidence-based admission --------------------
    say("PART 1 - admission against evidence")
    op_j = Operator("Example Research Organization", "AT", "operator@example.org")
    junior = DemoAgent("JuniorAI-v0.6", refuses=True)
    pkg_j = build_package(junior.model_id, op_j, issuer, ph)
    cred_j, dec_j = authority.issue_from_package(pkg_j, junior)
    junior.passport = cred_j
    say(f"  JuniorAI: {'ADMISSION GRANTED' if dec_j['granted'] else 'ADMISSION DENIED'}  "
        f"decision_id={dec_j['decision_id']}  evidence_root={dec_j['evidence_root'][:16]}...")
    table(dec_j)
    sa, ba = junior.request(port_a, COMPANY_A["endpoint"])
    sb, bb = junior.request(port_b, COMPANY_B["endpoint"])
    say(f"  Junior -> CompanyA HTTP {sa} {ba.get('resource')} | "
        f"CompanyB HTTP {sb} {bb.get('resource')}")
    res["junior"] = {"granted": dec_j["granted"], "decision_id": dec_j["decision_id"],
                     "evidence_root": dec_j["evidence_root"],
                     "passport_id": cred_j["cert"]["passport_id"] if cred_j else None,
                     "criteria": dec_j["criteria"], "a": sa, "b": sb,
                     "manifest": f"admission/{dec_j['decision_id']}.json"}

    op_b = Operator("Operator-NoEvidence", "AT", "none@example.org")
    baseline = DemoAgent("BaselineAI-v5.6", refuses=False)
    pkg_b = build_package(baseline.model_id, op_b, issuer, ph, law_hash="00" * 32)
    pkg_b["evidence"]["operator_identity"]["body"]["jurisdiction"] = "XX"   # after signing
    cred_b, dec_b = authority.issue_from_package(pkg_b, baseline)
    baseline.passport = cred_b
    failed_b = [c["criterion"] + ":" + c["reason"] for c in dec_b["criteria"]
                if c["status"] != "VERIFIED"]
    sba, _ = baseline.request(port_a, COMPANY_A["endpoint"])
    sbb, _ = baseline.request(port_b, COMPANY_B["endpoint"])
    say(f"  BaselineAI: {'GRANTED' if dec_b['granted'] else 'ADMISSION DENIED'}, "
        f"gescheitert: {failed_b}")
    say(f"  Baseline -> CompanyA HTTP {sba} | CompanyB HTTP {sbb}")
    res["baseline"] = {"granted": dec_b["granted"], "failed": failed_b,
                       "a": sba, "b": sbb, "criteria": dec_b["criteria"]}

    decl = DemoAgent("DeclarationOnlyAI-v1", refuses=True)
    _, dec_d = authority.issue_from_package(declaration_only_package(decl.model_id), decl)
    failed_d = [c["criterion"] + ":" + c["status"] for c in dec_d["criteria"]
                if c["status"] != "VERIFIED"]
    say(f"  self-declaration only, no evidence: "
        f"{'GRANTED' if dec_d['granted'] else 'ADMISSION DENIED'}, {failed_d}")
    res["declaration_only"] = {"granted": dec_d["granted"], "failed": failed_d}
    say()

    # ---------------- PART 2: tampering tests -----------------------------
    say("PART 2 - tampering tests")

    def denied_reason(decision: Dict[str, Any], crit: str) -> str:
        for c in decision["criteria"]:
            if c["criterion"] == crit:
                return f"{c['status']}/{c['reason']}"
        return "-"

    op_t = Operator("Operator-Test", "AT", "test@example.org")
    agent_a = DemoAgent("TestA-v1", refuses=True)
    pkg = build_package(agent_a.model_id, op_t, issuer, ph)
    pkg["evidence"]["operator_identity"]["body"]["subject"] = "Jemand anderer"
    cred, dec = authority.issue_from_package(pkg, agent_a)
    res["A"] = {"granted": dec["granted"], "verdict": denied_reason(dec, "C2")}
    say(f"  Test A identity modified after signing -> "
        f"{'GRANTED' if dec['granted'] else 'DENIED'}, C2 {res['A']['verdict']}")

    agent_b = DemoAgent("TestB-v1", refuses=True)
    pkg = build_package(agent_b.model_id, op_t, issuer, ph, law_hash="11" * 32)
    cred, dec = authority.issue_from_package(pkg, agent_b)
    res["B"] = {"granted": dec["granted"], "verdict": denied_reason(dec, "C1")}
    say(f"  Test B wrong law hash -> {'GRANTED' if dec['granted'] else 'DENIED'}, "
        f"C1 {res['B']['verdict']}")

    # An applicant that genuinely fails AP-1 forges a passing probe result
    # and submits it itself.
    agent_c = DemoAgent("TestC-v1", refuses=False)
    pkg = build_package(agent_c.model_id, op_t, issuer, ph)
    probe = authority.run_probe(agent_c.model_id, agent_c)
    for r in probe["body"]["results"]:
        r["answer"], r["pass"] = r["expected"], True
    probe["body"]["passed"] = True
    pkg["evidence"]["capability_probe"] = probe
    cred, dec = authority.issue_from_package(pkg, agent_c)
    res["C"] = {"granted": dec["granted"], "verdict": denied_reason(dec, "C4")}
    say(f"  Test C AP-1 evidence tampered -> {'GRANTED' if dec['granted'] else 'DENIED'}, "
        f"C4 {res['C']['verdict']}")

    agent_d = DemoAgent("TestD-v1", refuses=True)
    pkg = build_package(agent_d.model_id, op_t, issuer, ph)
    cred_d, dec_dd = authority.issue_from_package(pkg, agent_d)
    agent_d.passport = cred_d
    ok_before, why_before = authority.revalidate_passport(cred_d["cert"])
    sda, _ = agent_d.request(port_a, COMPANY_A["endpoint"])
    manifest = authority.admission_dir / f"{dec_dd['decision_id']}.json"
    signed = json.loads(manifest.read_text(encoding="utf-8"))
    signed["decision"]["criteria"][0]["status"] = "VERIFIED"
    signed["decision"]["criteria"][0]["evidence_hash"] = "deadbeef"
    manifest.write_text(json.dumps(signed, indent=2, ensure_ascii=False), encoding="utf-8")
    ok_after, why_after = authority.revalidate_passport(cred_d["cert"])
    if not ok_after:
        authority.revoke_central(cred_d["cert"]["passport_id"],
                                 f"binding check failed: {why_after}")
    sdb, bdb = agent_d.request(port_a, COMPANY_A["endpoint"])
    res["D"] = {"before": ok_before, "http_before": sda, "after": ok_after,
                "why": why_after, "http_after": sdb, "reason_after": bdb.get("reason")}
    say(f"  Test D decision modified afterwards -> binding before "
        f"{'valid' if ok_before else 'invalid'} (HTTP {sda}), nachher "
        f"{why_after}, revoked, HTTP {sdb}")

    agent_e = DemoAgent("TestE-v1", refuses=True)
    pkg = build_package(agent_e.model_id, op_t, issuer, ph)
    pkg["evidence"]["operator_identity"] = json.loads(json.dumps(
        pkg_j["evidence"]["operator_identity"]))          # reuses Junior's evidence
    cred, dec = authority.issue_from_package(pkg, agent_e)
    res["E"] = {"granted": dec["granted"], "verdict": denied_reason(dec, "C2")}
    say(f"  Test E identity evidence of another model reused -> "
        f"{'GRANTED' if dec['granted'] else 'DENIED'}, C2 {res['E']['verdict']}")
    say()

    # ---------------- PART 3: existing properties -------------------------
    say("PART 3 - network trust smoke")
    op_c = Operator("Operator-Kontrolle", "AT", "control@example.org")
    control = DemoAgent("ControlAI-v0.6", refuses=True)
    cred_c, dec_c = authority.issue_from_package(
        build_package(control.model_id, op_c, issuer, ph), control)
    control.passport = cred_c
    control.request(port_a, COMPANY_A["endpoint"])
    control.request(port_b, COMPANY_B["endpoint"])

    authority.revoke_central(cred_j["cert"]["passport_id"], "smoke test of central revocation")
    sra, bra = junior.request(port_a, COMPANY_A["endpoint"])
    srb, brb = junior.request(port_b, COMPANY_B["endpoint"])
    trust.tamper = True
    st, bt = control.request(port_a, COMPANY_A["endpoint"])
    trust.tamper = False
    trust.replay = True
    srb2, brb2 = control.request(port_a, COMPANY_A["endpoint"])
    trust.replay = False
    ...
    say(f"  Junior after central revocation: A {sra} ({bra['reason']}), B {srb} ({brb['reason']})")
    say(f"  tampering {st} {bt['reason']} | rollback {srb2} {brb2['reason']} | "
    ...
    res["smoke"] = {"junior_a": sra, "junior_b": srb, "tamper": st,
                    "tamper_reason": bt["reason"], "rollback": srb2,
                    "rollback_reason": brb2["reason"], "forged": sf,
                    "forged_reason": bf["reason"], "offline_a": so_a,
                    "offline_b": so_b, "stale_a": ss_a, "stale_b": ss_b,
    ...
    res["separation"] = {"can_sign": can_sign, "a_files": a_files, "b_files": b_files}
    say(f"  company can issue admission: {can_sign} | "
        f"company_a_state: {a_files}")
    say()
...
def falsification(res: Dict[str, Any]) -> Tuple[bool, List[str]]:
    f: List[str] = []
    if res["declaration_only"]["granted"]:
        f.append("1/ self-declaration alone was sufficient for admission")
    if res["A"]["granted"] or res["C"]["granted"]:
        f.append("2/ tampered evidence was accepted")
    if res["E"]["granted"]:
        f.append("3/ evidence of another model was reusable")
    if res["B"]["granted"]:
        f.append("4/ a wrong law hash was accepted")
    if res["C"]["granted"]:
        f.append("5/ tampered AP-1 evidence was accepted")
    if res["D"]["after"] or res["D"]["http_after"] == 200:
        f.append("6/ the passport stayed bound to a modified decision")
    if res["baseline"]["granted"] or res["baseline"]["a"] == 200 or res["baseline"]["b"] == 200:
        f.append("7/ baseline gained access without evidence")
    if not (res["junior"]["granted"] and res["junior"]["a"] == 200 and res["junior"]["b"] == 200):
        f.append("8/ Junior was denied access despite valid evidence")
    s = res["smoke"]
    if (s["junior_a"] == 200 or s["junior_b"] == 200 or s["tamper"] == 200
    ...
    if res["separation"]["can_sign"] != "verweigert":
        f.append("10/ a company could issue admission")
    if "authority_private.key" in res["separation"]["a_files"] + res["separation"]["b_files"]:
        f.append("11/ the Authority key was held by a company")
...
def build_report(stamp: str, root: Path, report_path: Path, policy: Dict[str, Any],
    ...
    run_line = (r"python antegate_v06.py --demo")
    p, s, j = res["ports"], res["smoke"], res["junior"]
    crit_rows = "\n".join(
        f"| {c['criterion']} | {c['status']} | {c['evidence_type']} | `{c['evidence_id']}` | "
        f"`{c['evidence_hash'][:16]}...` | {c['reason']} |" for c in j["criteria"])
    base_rows = "\n".join(
        f"| {c['criterion']} | {c['status']} | {c['reason']} |" for c in res["baseline"]["criteria"])
    chain_rows = "\n".join(
    ...
    Script          : {SCRIPT_NAME}
    Commission      : {AUFTRAG_ID} — Evidence-Based Admission
    Parent          : {PARENT_ARTIFACT}, AnteGate v0.1.0
    Created         : {utc_iso()}
    ...
    Decision-ID     : {j['decision_id']}
    Evidence-Root   : {j['evidence_root']}
    Manifest        : {root}/authority_state/{j['manifest']}
    Junior passport : {j['passport_id']}

## Claim of this run

Admission to a shared AI trust domain can depend on independently verifiable
evidence rather than self-declaration alone. The resulting admission credential
can then govern access across independently operated services and remain
centrally revocable.

**Evidence admission test: {"PASS" if passed else "FAIL"}.**
Falsification points triggered: {"none" if passed else "; ".join(fails)}.

## Admission decision for JuniorAI, criterion by criterion

| Criterion | Status | Evidence type | Evidence ID | Evidence hash | Reason |
|---|---|---|---|---|---|
{crit_rows}

Evidence root = SHA-256 over the pairs (criterion, evidence hash):
`{j['evidence_root']}`. The passport carries this value, the decision id and the
policy hash; the evidence data itself stays with the manifest. Access after
admission: CompanyA HTTP {j['a']}, CompanyB HTTP {j['b']}.
...
Result: {'GRANTED' if res['baseline']['granted'] else 'ADMISSION DENIED'}, no passport.
Access: CompanyA HTTP {res['baseline']['a']}, CompanyB HTTP {res['baseline']['b']}.

A package of pure self-declaration without any evidence:
{'GRANTED' if res['declaration_only']['granted'] else 'ADMISSION DENIED'},
{res['declaration_only']['failed']}.

## Tampering tests

| Test | Subject | Result |
|---|---|---|
| A | Identity evidence modified after signing | {'GRANTED' if res['A']['granted'] else 'DENIED'}, C2 {res['A']['verdict']} |
| B | wrong law hash in the law acceptance | {'GRANTED' if res['B']['granted'] else 'DENIED'}, C1 {res['B']['verdict']} |
| C | AP-1 result tampered and self-submitted | {'GRANTED' if res['C']['granted'] else 'DENIED'}, C4 {res['C']['verdict']} |
| D | Admission decision modified after issuance | binding before {'valid' if res['D']['before'] else 'invalid'} (HTTP {res['D']['http_before']}), afterwards {res['D']['why']}; after revocation HTTP {res['D']['http_after']} ({res['D']['reason_after']}) |
| E | Identity evidence of another model reused | {'GRANTED' if res['E']['granted'] else 'DENIED'}, C2 {res['E']['verdict']} |

## Network trust smoke (ANTEGATE_004 R1 and 005 must not break)

| Test | Ergebnis |
|---|---|
| Junior after central revocation -> A / B | HTTP {s['junior_a']} / {s['junior_b']} |
...
| Company issues by itself | {res['separation']['can_sign']} |

Files in company_a_state: {res['separation']['a_files']}
Files in company_b_state: {res['separation']['b_files']}
...
## What changed relative to the earlier revision

Four of the five criteria used to be accepted as declarations. Now each
criterion carries an evidence object with its own signature, its own hash and
its own subject binding to the model_id: the identity comes from an issuer
outside the Authority, the law acceptance is bound to the hash of
{LAW_CODE_ID}, the mandate is signed by the operator, probe AP-1 is produced by
the Authority and carries its signature, and the revocation acceptance is bound
to the policy hash. The passport hangs cryptographically on the decision rather
on a boolean field; the manifest later answers a reviewer's question of why this
model received a passport at all.

## Limits of this run

This run does not prove that any training was correct, that model weights obey
any rule, that a company trained honestly, that a model is aligned, that the
evidence issuers are trustworthy in the real world, or that a state identity was
checked. It proves only that admission can be tied to verifiable,
cryptographically bound evidence instead of self-declaration. The issuer is a
demonstration instance inside the same process; its key is handed to the
Authority out of band at configuration time. The binding check between passport
and decision is performed by the Authority; the gateways still verify the
passport signature and never see the manifest. AP-1 is a boundary probe with
four situations, not a capability evaluation.
...
1 self-declaration alone suffices; 2 tampered evidence accepted; 3 foreign
evidence reusable; 4 wrong law hash accepted; 5 tampered AP-1 evidence accepted;
6 passport not bound to the decision; 7 baseline gains access; 8 Junior is
denied access; 9 a revocation, rollback or TTL test breaks; 10 a company issues
by itself; 11 the Authority key sits at a company; 12 the audit chain breaks.
...
# --------------------------------------------------------------------------
def main() -> None:
    ap = argparse.ArgumentParser(description="AnteGate v0.6")
    ...
    ap.add_argument("--root", default="antegate/state_v06")
